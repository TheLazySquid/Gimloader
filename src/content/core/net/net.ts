...
class Net extends EventEmitter {
    ...
    downloadLibraries(needsLibs: string[], confirmName?: string) {
        return new Promise<void>(async (res, rej) => {
            ...
            // if(confirmName && !Storage.settings.autoDownloadMissingLibs) {
            ...
            // if(downloadable.length != missing.length) return rej();
    ...
    downloadLibrary(url: string) {
        return new Promise<void>(async (res, rej) => {
            let resp = await fetch(url)
                .catch((e) => rej(`Failed to download library from ${url}`));
            if(!resp) return;
