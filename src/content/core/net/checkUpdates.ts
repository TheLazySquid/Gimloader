...
export async function checkPluginUpdate(plugin: Plugin) {
    ...
    const res = await fetch(plugin.headers.downloadUrl)
        .catch(() => {
            alert(`Failed to download update from ${plugin.headers.downloadUrl}`);
        });
    if(!res) return;
    const text = await res.text();

    let incomingHeaders = parsePluginHeader(text);
    if(text === plugin.script) { 
...
export async function checkLibUpdate(lib: Lib) {
    ...
    const res = await fetch(lib.headers.downloadUrl)
        .catch(() => {
    ...
    let incomingHeaders = parseLibHeader(text);
    if(text === lib.script) { 
